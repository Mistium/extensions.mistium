(function (Scratch) {
  ...
  class roturVoice {
    constructor() {
      ...
      this._consoleLevel = 1;
      this._consoleProxy = {
        log: (...a) => this._shouldLog(3) && this._originalConsole.log(...a),
        warn: (...a) => this._shouldLog(2) && this._originalConsole.warn(...a),
        error: (...a) => this._shouldLog(1) && this._originalConsole.error(...a)
      };
    }
        ...
        blocks: [
          {
            ...
            text: 'connect to voice server as [NAME]',
            arguments: {
              NAME: {
          ...
          },
          {
            ...
            blockType: Scratch.BlockType.COMMAND,
            text: 'disconnect from voice server'
          },
          {
            opcode: 'isConnected',
          ...
          "---",
          {
            opcode: 'setLogging',
            blockType: Scratch.BlockType.COMMAND,
            text: 'set voice debug logging to [LEVEL]',
            arguments: {
              LEVEL: {
                type: Scratch.ArgumentType.STRING,
                menu: 'logLevel',
          ...
          },
          {
            opcode: 'hasAudio',
            blockType: Scratch.BlockType.BOOLEAN,
            text: 'is my microphone enabled?'
          },
          "---",
          {
            opcode: 'callPeer',
            blockType: Scratch.BlockType.COMMAND,
            text: 'call [NAME] and [WAIT] (call id: [CALLID])',
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'friend'
          ...
          },
          {
            opcode: 'answerCall',
            blockType: Scratch.BlockType.COMMAND,
            text: 'answer call with id [CALLID]',
            arguments: {
              CALLID: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'default'
          ...
          },
          {
            opcode: 'hangup',
            blockType: Scratch.BlockType.COMMAND,
            text: 'hang up call with id [CALLID]',
            arguments: {
              CALLID: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'default'
          ...
          {
            opcode: 'isCallIncoming',
            blockType: Scratch.BlockType.BOOLEAN,
            text: 'is there an incoming call? (id: [CALLID])',
            arguments: {
              CALLID: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'default'
          ...
          },
          {
            ...
            blockType: Scratch.BlockType.REPORTER,
            text: 'partner name for call [CALLID]',
            arguments: {
              CALLID: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: 'default'
    ...
    _log(...args) { this._consoleProxy?.log(...args); }
    _warn(...args) { this._consoleProxy?.warn(...args); }
    _error(...args) { this._consoleProxy?.error(...args); }
    ...
    async connect(args) {
      try {
        ...
        this._error('Error connecting:', error);
        this.connectionStatus = 'error: ' + (error.message || 'unknown');
      }
    }
    ...
    async callPeer(args) {
      ...
      return new Promise(async (resolve) => {
        ...
        if (this.connectionStatus !== 'connected') {
          this._error(`Cannot call - connection status is ${this.connectionStatus}`);
          this.callStatuses[callId] = 'error: not connected';
          resolve();
          return;
        }
        ...
        if (!this.audioStream) {
          ...
          if (!audioEnabled || !this.audioStream) {
            this._error('Could not enable audio for call');
            this.callStatuses[callId] = 'error: no audio';
            resolve();
            return;
          }
        }
        ...
        if (!name) {
          this._error('Invalid name to call');
          this.callStatuses[callId] = 'error: invalid name';
          this.callPartners[callId] = '';
          resolve();
          return;
        }

        this.callStatuses[callId] = 'connecting';
        this.callPartners[callId] = name;
        ...
        if (!call) {
          ...
          this._error('Failed to initiate call - call object not created');
          this.callStatuses[callId] = 'failed';
          this.callPartners[callId] = '';
          resolve();
          return;
        }
    ...
    _setupCallEvents(callId, call, callTimeout) {
      call.on('stream', (remoteStream) => {
        ...
        try {
          const audioElementId = 'roturCallAudio_' + callId;
          ...
          this._error('Error setting up audio element:', e);
        }
      });
      call.on('close', () => {
        if (callTimeout) clearTimeout(callTimeout);
        ...
        try {
          ...
          this._error('Error removing audio element:', e);
        }
        delete this.calls[callId];
    ...
    answerCall(args) {
      ...
      if (!callId || !this.incomingCalls[callId]) {
        this._log('No incoming call to answer for id', callId);
        return;
      }
      if (!this.audioStream) {
        this.enableAudio({ mic: 'enable' }).then(() => {
          if (this.audioStream) {
            this._completeAnswerCall(callId);
          } else {
            this._error('Could not enable audio to answer call');
          }
        });
      } else {
        this._completeAnswerCall(callId);
    ...
    _completeAnswerCall(callId) {
      if (!this.incomingCalls[callId]) return;
      const call = this.incomingCalls[callId];
      this.calls[callId] = call;
      this.callPartners[callId] = call.peer || '';
      this.callStatuses[callId] = 'connecting';
      call.answer(this.audioStream);
      this._setupCallEvents(callId, call);
    ...
    isConnected() {
      return Boolean(this.peer && this.connectionStatus === 'connected');
    }

    onCall(args) {
      const callId = this._resolveCallId(args.CALLID);
      return Boolean(callId && this.calls[callId] && this.callStatuses[callId] === 'connected');
    }

    isCallIncoming(args) {
      const callId = this._resolveCallId(args.CALLID);
      return Boolean(callId && this.callStatuses[callId] === 'incoming');
    }
    ...
    getCallStatus(args) {
      const callId = this._resolveCallId(args.CALLID);
      return String((callId && this.callStatuses[callId]) || 'unknown');
    }
    ...
    getCallPartner(args) {
      const callId = this._resolveCallId(args.CALLID);
      return String((callId && this.callPartners[callId]) || '');
    }

    getAllCallIds() {
      const ids = new Set([
        ...Object.keys(this.calls),
        ...Object.keys(this.incomingCalls)
      ]);
