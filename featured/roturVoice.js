...
(function (Scratch) {
  ...
  class roturVoice {
      ...
      const call_promise = new Promise(async (resolve) => {
        try {
          ...
          if (this.connectionStatus !== 'connected') {
            console.error(`Cannot call - connection status is ${this.connectionStatus}`);
            this.callStatus = 'error: not connected';
            resolve();
            return;
          }
          ...
          if (!name) {
            console.error('Invalid name to call');
            this.callStatus = 'error: invalid name';
            this.callPartner = '';
            resolve();
            return;
          }
          ...
          if (!this.call) {
            ...
            console.error('Failed to initiate call - call object not created');
          ...
          this.callStatus = 'error: ' + (error.message || 'unknown');
          this.callPartner = '';
          resolve();
        }
      });
    ...
    _setupCallEvents(callTimeout) {
      ...
      this.call.on('stream', (remoteStream) => {
        ...
        try {
          ...
          const existingAudio = document.getElementById('roturCallAudio');
          if (existingAudio) {
    ...
    _triggerCallChangeEvent() {
      if (this.onCallChangeCallback) {
        try {
          this.onCallChangeCallback(this.callStatus);
        } catch (e) {
          console.error('Error in call change callback:', e);
        }
      }
    }
    ...
    onCall() {
      return Boolean(this.call && this.callStatus === 'connected');
    }

    isCallIncoming() {
      return Boolean(this.callStatus === 'incoming');
    }
    ...
    getCallStatus() {
      return String(this.callStatus || 'unknown');
    }
    ...
    getCallPartner() {
      return String(this.callPartner || '');
    }
  }
